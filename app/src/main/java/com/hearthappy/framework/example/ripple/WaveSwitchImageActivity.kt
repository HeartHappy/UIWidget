...
class WaveSwitchImageActivity: AbsBaseActivity<ActivityWaterRippleViewerBinding>() {
    ...
    override fun ActivityWaterRippleViewerBinding.initView() {
        rvWaterRipple.apply {
            ...
            //            }
            //        }
            setAdapter(object: WaterRippleView.WaterRippleAdapter {
                override fun onBindView(data: String, imageView: ImageView, position: Int) {
                    Glide.with(this@WaveSwitchImageActivity).load(data).into(imageView)
                }
            }) //        startCarousel()
            initData(ImageUtil.urlsData)
